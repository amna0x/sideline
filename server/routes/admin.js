...
import { z } from 'zod'
import { mode, getUser, createUser, getVaultItems } from '../db/index.js'
  ...
  try {
    ...
    if (!user) {
      user = await createUser({
        id: user_id,
        username: `user_${user_id.slice(0, 6)}`,
        tier: 'fan',
        points_total: 0,
        predictions_made: 0,
        predictions_correct: 0,
        matches_watched: 0,
        prediction_title: 'Rookie'
      })
    }
    ...
    res.json({ ok: true, user, points_total: user.points_total || 0, granted_count: granted.length, granted })
  } catch (e) { next(e) }
})
