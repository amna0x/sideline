...
r.get('/:id', optionalAuth, async (req, res, next) => {
  try {
    ...
    // Update username from auth token if the DB username looks auto-generated
    if (req.user?.username && req.user.id === id) {
      const tokenUsername = req.user.username
      const dbUsername = user.username || ''
      const looksAutoGenerated = !dbUsername || dbUsername.startsWith('op_') || dbUsername === (user.email || '').split('@')[0]
      if (looksAutoGenerated && tokenUsername && !tokenUsername.includes('@') && tokenUsername.length <= 30) {
        user = await updateUser(id, { username: tokenUsername })
      }
    }
